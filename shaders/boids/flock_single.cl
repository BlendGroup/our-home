...
float4 cohesion(__global const boid_t *flock, const size_t size, const int this_id, const float radius, const float max_force) {
    float4 force = (float4)(0.0f, 0.0f, 0.0f);
...
float4 alignment(__global const boid_t *flock, const size_t size, const int this_id, const float radius, const float max_force) {
    float4 force = (float4)(0.0f, 0.0f, 0.0f);
    const boid_t *current = &flock[this_id];
    int count = 0;
    for(int other_id = 0; other_id < size; other_id++) {
...
float4 separation(__global const boid_t *flock, const size_t size, const int this_id, const float radius, const float max_force) {
    float4 force = (float4)(0.0f, 0.0f, 0.0f);
    const boid_t *current = &flock[this_id];
    int count = 0;
    for(int other_id = 0; other_id < size; other_id++) {
...
float4 attraction(__global const boid_t *flock, const int this_id, const float4 attractor_position, const float max_force) {
...
__kernel void flock_update (
    ...
    float4 acceleration = (float4)(0.0f, 0.0f, 0.0f);
