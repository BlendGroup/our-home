...
GLuint opensimplexnoise::createNoiseTexture2D(vmath::ivec2 dim, vmath::ivec2 offset, float timeInterval, float amplitude, long seed) {
	...
	programglobal::oclContext->setKernelParameters(noiseKernel, { clkernelparam(0, inputGrid), clkernelparam(1, outputNoiseCLGL.cl), clkernelparam(2, pointCount), clkernelparam(3, amplitude) });
...
GLuint opensimplexnoise::createFBMTexture2D(vmath::ivec2 dim, vmath::ivec2 offset, float timeInterval, float amplitude, int octaves, long seed) {
	...
	clglmem outputNoiseCLGL;
	outputNoiseCLGL = programglobal::oclContext->createCLGLBuffer(sizeof(float) * dim[0] * dim[1], GL_MAP_WRITE_BIT | GL_MAP_READ_BIT, CL_MEM_READ_WRITE);
	programglobal::oclContext->setKernelParameters(noiseKernel, { clkernelparam(0, inputGrid), clkernelparam(1, outputNoiseCLGL.cl), clkernelparam(2, pointCount), clkernelparam(3, octaves), clkernelparam(4, amplitude) });
	programglobal::oclContext->runCLKernel(noiseKernel, 1, &globalWorkSize, NULL, { outputNoiseCLGL });
...
GLuint opensimplexnoise::createTurbulenceFBMTexture2D(vmath::ivec2 dim, vmath::ivec2 offset, float timeInterval, int octaves, float noiseoffset, long seed) {
	...
	clglmem outputNoiseCLGL;
	outputNoiseCLGL = programglobal::oclContext->createCLGLBuffer(sizeof(float) * dim[0] * dim[1], GL_MAP_WRITE_BIT | GL_MAP_READ_BIT, CL_MEM_READ_WRITE);
	programglobal::oclContext->setKernelParameters(noiseKernel, { clkernelparam(0, inputGrid), clkernelparam(1, outputNoiseCLGL.cl), clkernelparam(2, pointCount), clkernelparam(3, octaves), clkernelparam(4, noiseoffset) });
	programglobal::oclContext->runCLKernel(noiseKernel, 1, &globalWorkSize, NULL, { outputNoiseCLGL });
	CLErr(clhelpererr = clFinish(programglobal::oclContext->getCommandQueue()));
	glGenTextures(1, &outputNoise);
	glBindTexture(GL_TEXTURE_2D, outputNoise);
...
GLuint opensimplexnoise::combineTwoNoiseTextures(GLuint inputTex1, GLuint inputTex2, ivec2 dim, float offset) {
	...
	programglobal::oclContext->setKernelParameters(combineKernel, {clkernelparam(0, inputImage1.cl), clkernelparam(1, inputImage2.cl), clkernelparam(2, outputImage.cl), clkernelparam(3, offset)});
