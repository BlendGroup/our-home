...
opensimplexnoise::opensimplexnoise() {
	try {
		string programName;
		programglobal::oclContext->compilePrograms({"shaders/opensimplexnoisenew.cl"});
...
void create2DNoiseTexture(cl_kernel kernel, cl_mem inputGrid, clglmem outputNoise, ivec2 dim, float amplitude, long seed) {
	try {
		...
		programglobal::oclContext->setKernelParameters(kernel, { param(0, inputGrid), param(1, outputNoise.cl), param(2, pointCount), param(3, amplitude) });
